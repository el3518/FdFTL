...
class Classifier_Rb(nn.Module):
    def __init__(self, feature_dim, rule_num, type="linear"):
        super(Classifier_Rb, self).__init__()
        
        self.type = type
        if type == 'wn':
            self.fc = nn.utils.weight_norm(nn.Linear(feature_dim, rule_num), name="weight")
            self.fc.apply(init_weights)
        else:
            self.fc = nn.Linear(feature_dim, rule_num)
            self.fc.apply(init_weights)
...
class Classifier_R(nn.Module):
    def __init__(self, embed_dim, rule_num, type="linear"):
        super(Classifier_R, self).__init__()
        
        self.type = type
        if type == 'wn':
            self.fc = nn.utils.weight_norm(nn.Linear(embed_dim, rule_num), name="weight")
            self.fc.apply(init_weights)
        else:
            self.fc = nn.Linear(embed_dim, rule_num)
            self.fc.apply(init_weights)
...
class Classifier(nn.Module):
    def __init__(self, embed_dim, class_num, rule_num=65, type="linear"):
        ...
        for i in range(rule_num):
            if type == 'wn':
                self.fc[i] = nn.utils.weight_norm(nn.Linear(embed_dim, class_num), name="weight")
                self.fc[i].apply(init_weights)
            else:
                self.fc[i] = nn.Linear(embed_dim, class_num)
                self.fc[i].apply(init_weights)
...
class SFUniDA(nn.Module):
    def __init__(self, args):
        ...
        self.class_layer = Classifier(self.embed_feat_dim, class_num=self.class_num, rule_num = self.rule_num, type="wn")
        
        #self.rule_layer = Classifier_R(self.embed_feat_dim, rule_num = self.rule_num, type="wn")
        self.rule_layer = Classifier_Rb(self.backbone_feat_dim, rule_num = self.rule_num, type="wn")
    ...
    def forward(self, input_imgs, apply_softmax=True):
        ...
        #rule_out = self.rule_layer(embed_feat)
        rule_out = self.rule_layer(backbone_feat)
        
        if apply_softmax:
            cls_out = torch.softmax(cls_out, dim=1)
        
        return backbone_feat, embed_feat, cls_out, rule_out
