...
def clu_anc_para(net, args):#netC=netC1
    ...
    for i in range(len(para)):#i=0
        parai= para[i]
        parai = (parai / torch.norm(parai, p=2, dim=1, keepdim=True)).numpy()
        anc = anc + parai
    anc = anc / len(para) 
    return anc
...
def cal_output_sel(output, member, tar_rule, args):    
    outputs = torch.zeros([output[0].shape[0], args.class_num]).cuda()
    for i in range(args.sel_rule_num): #rule_num
...
def obtain_global_pseudo_labels(args, model, dataloader, epoch_idx=0.0):
    ...
    for _, imgs_test, imgs_label, _ in tqdm(dataloader, ncols=60):
        ...
        backb_feat, embed_feat, pred_cls, _ = model(imgs_test, apply_softmax=False)
        
        mem_ship, tar_rule = fuz_mem(backb_feat, args.cen, args)  
        pred_cls = torch.softmax(cal_output_sel(pred_cls, mem_ship, tar_rule, args), dim=1)  
        #pred_cls_final = cal_output_sel(pred_cls, mem_ship, tar_rule, args)
    ...
    #pos_topk_num = int(2*data_num / args.class_num / best_coeff)
    #pos_topk_num = int(0.5*data_num / args.class_num / best_coeff)
...
def train(args, model, train_dataloader, test_dataloader, optimizer, epoch_idx=0.0):
    ...
    for imgs_train, _, _, imgs_idx in tqdm(train_dataloader, ncols=60):
        ...
        backb_feat, embed_feat, pred_cls, _ = model(imgs_train, apply_softmax=False)
...
def test(args, model, dataloader, src_flg=False):
    ...
    for _, imgs_test, imgs_label, _ in tqdm(dataloader, ncols=60):
        ...
        imgs_test = imgs_test.cuda() 
        backb_feat, _, pred_cls, _ = model(imgs_test, apply_softmax=False)
...
#############################################################################    
    model = model.cuda()
    save_dir = os.path.join(this_dir, "checkpoints_glc_fuz", args.dataset, str(args.rule_num), "adpt_v1_target_{}".format(args.t_idx),
    ...
    ##############################################
    args.cen = clu_anc_para(model.rule_layer, args)
    ...
    notation_str =  "\n=======================================================\n"
    notation_str += "   START TRAINING ON THE TARGET:{} \n".format(args.t_idx)
    notation_str += "======================================================="
